...
option_list <-  list(
  ...
  make_option("--height", default=32,
              help="The heigth of the png to save, in cm."),
  make_option("--width", default=36,
...
if(!is.null(opt$csvpaths)) {
  ...
  paths <- gsub("filtered_feature_bc_matrix","metrics_summary.csv",caf$filtered_path)
  paths <- gsub("\\/$","", paths)
  ...
  listfile <- list()
  for(n in paths){ print(n); listfile[[n]] <- read.csv(n, header=T, stringsAsFactors = F) }
...
if(opt$kneeplot){
  ...
  ggsave(gb,height = 15 ,width = 17, dpi=300, file=paste0(opt$figdir,"10xMetrics_",project,"_knee_plot.png"), type="cairo-png")
